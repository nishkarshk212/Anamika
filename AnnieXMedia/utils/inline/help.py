...
def generate_help_buttons(_, start: int, end: int, current_page: int):
    """Create a grid of three buttons per row. Skips removed sections."""
    buttons, per_row = [], 3
    skipped_sections = {7, 17, 28, 29}  # Extra, Masti, Tag-All, Text Editing removed
...
def second_page(_):
    buttons = generate_help_buttons(_, 16, 26, current_page=2)  # Only up to 26 now
    buttons.append(
        [
